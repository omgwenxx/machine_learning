...
def invertModel(model, lrMod, lrInv, nStep=20, plot=False, verbose=False, show=False, save=False):
...
def reconstructionAttack(model, alpha = 5000, beta = 100, gamme = 0.1, delta = 0.1, save = True, show = False):
    ...
    for c in classes:
        ...
        if (show or save):
            ...
            ax2.imshow(orig, cmap='gray')
            if save:
                plt.savefig(f'./data/results/'+model.name+'class_{c}.png')
            if show:
                plt.show()
